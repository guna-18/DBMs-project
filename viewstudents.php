...
<a href="logout.php" class="w3-bar-item w3-button">logout</a>
	...
	<a href="fahome.php" class="w3-bar-item w3-button  w3-hover-green w3-right"> Back </a>
    ...
    <button class="w3-button w3-hover-green">Batch</button>
    <div class="w3-dropdown-content">
      <a href="#B" class="w3-bar-item w3-button w3-hover-green" onclick="document.getElementById('B').style.display='block'
		;document.getElementById('all').style.display='none'
		;document.getElementById('fail').style.display='none'
		;document.getElementById('btwcg').style.display='none'
		;document.getElementById('med').style.display='none'
	  ...
	  <a href="#J" class="w3-bar-item w3-button w3-hover-green" onclick="document.getElementById('J').style.display='block'
		;document.getElementById('all').style.display='none'
		;document.getElementById('fail').style.display='none'
		;document.getElementById('btwcg').style.display='none'
		;document.getElementById('B').style.display='none'
...
<?php
	if(isset($_POST['cgpa']))
	{
		...
		$cgpa1=$_POST['cgpa1'];
		$cgpa2=$_POST['cgpa2'];
		$sql="SELECT * FROM student_record,grade_report WHERE cgpa>='$cgpa1' AND cgpa<='$cgpa2' AND (student_record.student_login_roll_number = grade_report.student_login_roll_number) AND 
		student_record.joinyear='$year'";
		$result=$conn->query($sql);
		if($result->num_rows>0){
...
<?php
	...
	include("database.php");
	$sql="SELECT name,student_login_roll_number,fa_name FROM student_details WHERE joinyear='$year'";
	$result=$conn->query($sql);
	if($result->num_rows>0)
	{
...
<div id="fail" style="display:none" >
	<?php
		$sql="SELECT * FROM student_details WHERE joinyear='$year' ";
		$result=$conn->query($sql);
...
<?php
	$sql="SELECT * FROM student_record WHERE joinyear='$year' AND disciplinary_action IS NOT NULL";
	$result=$conn->query($sql);
	if($result->num_rows>0)
	{
		echo "<table class='w3-table w3-striped w3-hoverable' style='width:50%' align='center'>";
		echo "<br>"."<br>"."<br>";			
		echo "<tr>"."<th>"."Roll Number"."</th>"."<th>"."Disciplinary action taken"."</th>"."</tr>";
		while($row=$result->fetch_assoc())
		{
			echo "<tr>"."<td>".$row['student_login_roll_number']."</td>"."<t>".$row['disciplinary_action']."</td>"."</tr>";
		}
		echo "</table>";
	}
	else
	{
		echo "<br><br><center>No students to display</center>";
	}
...
<br>
<center><a href="fahome.php" class="w3-button w3-white w3-border w3-hover-blue-grey">  Back </a></center>
